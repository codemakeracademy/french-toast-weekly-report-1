                ...
                <div className="col-md-9">
                    ...
                    <ul className="list-group">
                        {teamMemberStore.map((item, index) => (
                                <li key={index} className="shadow-sm rounded list-group-item mb-2 d-flex border-1 align-items-center justify-content-between text-center">
                                    <div className="d-flex align-items-center">
                                        <div className="round p-2 rounded-circle">{item.split(" ").map((n)=>n[0]).join("")}</div>
                                        <div className="m-3">{item}</div>
                                    </div>
                                    <NavLink onClick={()=>onClickEdit(item)} className="text-decoration-none text-reset" to="/edit-member-information">Edit</NavLink>
                                </li>
                            )
                        )}
                    </ul>
